...
if [ "$GC_CMSSWRUN_RETCODE" == "0" ] && [ -n "$CMSSW_CONFIG" ]; then
	...
	for CFG_NAME in $CMSSW_CONFIG; do
		...
		echo "Starting python with config file $CFG_NAME and arguments $@"
		...
		[ "$CODE" == "" ] && export CODE="-2"
		echo "python finished with exit code $CODE"
		echo
		timestamp "CMSSW_CMSRUN${_CMSRUN_COUNT}" "DONE"
		_CMSRUN_COUNT=$[ $_CMSRUN_COUNT +1]
		if [ "$CODE" != "0" ];then
			echo "CMSSWLight config $CFG_NAME failed with code: $CODE"
			echo "Aborting..."
			break
		fi
	done
